...
const checkedFields = () => {
    const elements = $("formEditLicitacion").elements;
    $("errorFormEditLicitacion").innerHTML = null;
  
    for (let i = 0; i < elements.length - 2; i++) {
      if (elements[i].classList.contains("is-invalid")) {
        $("errorFormEditLicitacion").innerHTML = "Hay campos con errores o están vacíos";
      }
    }
  };
...
$('formEditLicitacion').addEventListener('submit', function(e){
    e.preventDefault();
    ...
    for (let i = 0; i < this.elements.length -1; i++) {
        ...
        if(!this.elements[i].value.trim() || this.elements[i].classList.contains('errorInput')) {
            ...
            $('errorFormEditLicitacion').innerHTML = "Hay campos vacíos o con errores!"
        }
